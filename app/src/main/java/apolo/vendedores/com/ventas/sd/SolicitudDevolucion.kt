...
class SolicitudDevolucion : AppCompatActivity() {
    ...
    lateinit var listaProductos :  ArrayList<HashMap<String,String>>
    lateinit var listaDetalles  :  ArrayList<HashMap<String,String>>
    lateinit var listaEnviados  :  ArrayList<HashMap<String,String>>
    lateinit var fspMotivo : FuncionesSpinner
    lateinit var fspUM : FuncionesSpinner
    ...
    private fun inicializaETAccion(etAccion: EditText){
        etAccion.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(s: Editable?) {
                if (etAccion.text.toString().equals("CANTIDAD")){
                    etAccion.setText("")
                }
                if (etAccion.text.toString().trim().equals("eliminarDetalle")){
                    eliminarDetalle(listaDetalles.get(posDetalles).get("id").toString().trim())
                    etAccion.setText("")
                }
                if (etAccion.text.toString().trim().equals("SALIR")){
                    eliminarTodoDetalle()
                    finish()
                }
                if (etAccion.text.toString().trim().equals("ACTUALIZAR")){
                    etAccion.setText("")
                    cargarEnviado()
                    cargarDetalle()
                }
    ...
    private fun cargarProductos(){
        ...
        listaProductos = ArrayList<HashMap<String,String>>()
    ...
    private fun mostrarProductos(){
        ...
        lvProductos.setOnItemClickListener { parent: ViewGroup, view: View, position: Int, id: Long ->
            FuncionesUtiles.posicionCabecera = position
            posProducto = position
            view.setBackgroundColor(Color.parseColor("#aabbaa"))
            etDetCodArticulo.setText(listaProductos.get(posProducto).get("COD_ARTICULO"))
            unidadDeMedida()
    ...
    private fun motivos(){
        var campos : String = " COD_EMPRESA,COD_MOTIVO,DESC_MOTIVO "
        var tabla : String = " svm_motivos_sd_dev "
        var where : String = " COD_EMPRESA = '1' "
        var whereOpcional : String = if (true) { "" } else { "" }
        var group : String = ""
        var order : String = ""
    ...
    private fun unidadDeMedida(){
        ...
        val whereOpcional : String = if (true) { "" } else { "" }
        val group = ""
        val order = ""
    ...
    private fun registrarSDDetalle(): Boolean {
        var resultado = false
        var orden: String
        orden = (funcion.ultimoNroOrden("svm_solicitud_dev_det") + 1).toString() + ""
        MainActivity.bd!!.beginTransaction()

        val misValues = ContentValues()
        misValues.put("NRO_ORDEN", orden)
        misValues.put("COD_EMPRESA", listaProductos.get(posProducto).get("COD_EMPRESA"))
        misValues.put("NRO_PLANILLA", FuncionesUtiles.usuario.get("LOGIN"))
        ...
        misValues.put("COD_ARTICULO", listaProductos.get(posProducto).get("COD_ARTICULO"))
        misValues.put("DESC_ARTICULO", listaProductos.get(posProducto).get("DESC_ARTICULO"))
        ...
        try {
            val cColumnas: String
            cColumnas = (" NRO_ORDEN,COD_EMPRESA,NRO_PLANILLA,COD_VENDEDOR,COD_CLIENTE,COD_SUBCLIENTE,NRO_REGISTRO_REF,COD_ARTICULO,"
    ...
    private fun validarRegistro():Boolean{
        ...
        if (etDetCantidad.text.toString().trim().equals("0")){
    ...
    private fun verificarExistencia():Boolean{
        val sql : String = ("SELECT COD_EMPRESA,NRO_PLANILLA,COD_VENDEDOR,COD_CLIENTE,COD_SUBCLIENTE "
                         ...
                         + "  WHERE NRO_PLANILLA 	= '${FuncionesUtiles.usuario.get("LOGIN")}' "
                         + "    AND COD_SUBCLIENTE	= '" + etCodCliente.text.toString().split("-")[1].trim() + "' "
                         + "    AND COD_CLIENTE 	= '" + etCodCliente.text.toString().split("-")[0].trim() + "' "
                         + "    AND COD_ARTICULO	= '" + listaProductos.get(posProducto).get("COD_ARTICULO") + "' "
                         + "    AND EST_ENVIO		= 'N'"
    ...
    private fun cantidadDeRegistros():Int{
        var sql : String = ("SELECT COD_EMPRESA,NRO_PLANILLA,COD_VENDEDOR,COD_CLIENTE,COD_SUBCLIENTE "
                + "  FROM svm_solicitud_dev_det "
                + " WHERE NRO_PLANILLA 	= '${FuncionesUtiles.usuario.get("LOGIN")}' "
                + "   AND COD_SUBCLIENTE	= '" + etCodCliente.text.toString().split("-")[1].trim() + "' "
                + "   AND COD_CLIENTE 	= '" + etCodCliente.text.toString().split("-")[0].trim() + "' "
                + "   AND EST_ENVIO		= 'N'")
        try {
            return funcion.consultar(sql).count
    ...
    private fun enviarSD(){
        ...
        etAccion = accion
        var enviarSD : EnviarSD = EnviarSD()
        enviarSD.enviar()
    }

    //Detalle
    private fun cargarDetalle(): Unit {
        listaDetalles = ArrayList<HashMap<String, String>>()
        ...
        val sql = ("select id,(NRO_ORDEN*1) NRO_ORDEN,COD_EMPRESA,COD_ARTICULO,DESC_ARTICULO,"
                        ...
                        + " NRO_PLANILLA	= '${FuncionesUtiles.usuario.get("LOGIN")}' and"
                        + " EST_ENVIO 		= 'N' "
                        + " ORDER BY NRO_ORDEN")
    ...
    private fun mostrarDetalles(){
        ...
        lvDetalle.setOnItemClickListener { parent: ViewGroup, view: View, position: Int, id: Long ->
    ...
    private fun eliminarDetalle(id:String){
        var sql : String = "DELETE FROM svm_solicitud_dev_det WHERE id = $id "
    ...
    private fun registrosPendientes():Boolean{
        var sql = "SELECT COD_EMPRESA,NRO_PLANILLA,COD_VENDEDOR,COD_CLIENTE,COD_SUBCLIENTE FROM svm_solicitud_dev_det WHERE EST_ENVIO	= 'N'"
        if (funcion.consultar(sql).count>0){
            return true
        }
        return false
    ...
    override fun onBackPressed() {
        if (registrosPendientes()){
            var mensaje = "Al salir se perderán las solicitudes pendientes de envio.\n¿Deseas continuar?"
    ...
    //Enviados
    private fun cargarEnviado(): Unit {
        listaEnviados = ArrayList<HashMap<String, String>>()
    ...
    private fun mostrarEnviados(){
        ...
        lvEnviado.setOnItemClickListener { parent: ViewGroup, view: View, position: Int, id: Long ->
